...
def load_model(config):
    if os.path.exists("aychang"):
        shutil.rmtree("aychang")
...
def train_model(model, criterion, optimizer, dataloaders, 
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            running_loss = 0.0
            running_corrects = 0
            for input_ids, attention_masks, labels in tqdm(dataloaders[phase]):
...
def run_model(config):
    ...
    earlyStopper = EarlyStopper(patience=3, min_delta=1e-2)
